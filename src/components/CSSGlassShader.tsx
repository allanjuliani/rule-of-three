...
export default function CSSGlassShader({
  backgroundMedia,
  uniforms,
  className,
      ...
      if (backgroundMedia.type === "image") {
        ...
        img.onerror = () => {
          ...
          if (backgroundMedia.blurhash) {
            try {
              const canvas = decodeToCanvas(
                backgroundMedia.blurhash,
                64,
                64,
                1,
              );
  ...
  useEffect(() => {
    ...
    const glassStyles: any = {
      ...
      clipPath: getShapeClipPath(
        uniforms.shape,
        uniforms.donutThickness,
        uniforms.starPoints,
        uniforms.starInnerRadius,
      ),
    };
