...
import CSSGlassShader from './CSSGlassShader';
...
export default function LiquidGlassShader({ backgroundMedia, uniforms, className, isTransitioning = false, onReady }: LiquidGlassShaderProps) {
  ...
  useEffect(() => {
    if (webglSupported === false) {
      // WebGL not supported, let CSS fallback handle it
      return;
    }

    const canvas = canvasRef.current;
    if (!canvas || webglSupported === null) return;
  ...
  }, [backgroundMedia, createShader, createProgram, loadTexture, createWhiteTexture, fragmentShaderSource, cacheUniformLocations, webglSupported]);
